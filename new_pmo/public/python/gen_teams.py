...
def generate_page_team(document, page_tree):
    if document is not None and page_tree is not None:
        global gl_font_size

        panels = page_tree.xpath("//acrich-text-body/child::h1")
        for p in panels:
            # Clean title from CCF formatting
            heading = p.text.replace("*", "")
            if heading == "Produkty, które utrzymujemy":
                # Generate text
                text_body = p.xpath("../following-sibling/*")
                for tb in text_body:
...
if __name__ == '__main__':
    ...
    teams_page = confl.get_page_by_id("38054088")
    if teams_page is not None:
        # get child pages and sort them
        child_pages = confl.get_child_pages(teams_page)
        teams = {}
        for page in child_pages:
            ...
            team_string = generate_page_team(document, page_tree)
            teams[title_doc] = team_string
