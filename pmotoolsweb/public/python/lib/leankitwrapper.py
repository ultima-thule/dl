...
class LeankitConnector(object):
    def __init__(self, account, username=None, password=None, throttle=0.5):
    ...
    def _do_request(self, action, url, data=None, handle_errors=True):
        ...
        try:
            if action == 'POST':
                resp = requests.post (self.base_api_url + url, auth=self.http.auth, data=data)
            else:
...
class LeankitCard(Converter):
    ...
    def fetchSubtasks(self, board):
        ...
        url = '/v1/board/{0}/card/{1}/tasks'.format(board.id, self.id)

        self.tasks = []
        try:
            taskboard_data = board.connector.get(url).ReplyData[0]
...
class LeankitBoard(Converter):
    ...
    def fetchDetails(self):
        ...
        for master_lane in self._archive[0]['ChildLanes']:
            archive_lanes.extend(
                [lane_dict['Lane'] for
                lane_dict in master_lane['ChildLanes']])

        self._archiveOldCards = []
        for i in range (1, 12):
            searchOptions = {
                "SearchTerm": "",
            ...
            oldData = self.connector.post("/Board/" + str(self.id) + "/searchcards", searchOptions).ReplyData[0]
            for cardItem in oldData['Results']:
                card = LeankitCard (cardItem, None)
                self.cards.append(card)

        self._populateUsers(self.details['BoardUsers'])
        self._populateCardTypes(self.details['CardTypes'])
        self._populateLanes(archive_lanes)
        # self._populateLanes(self.details['Lanes'] + self._backlog + self._archive)
